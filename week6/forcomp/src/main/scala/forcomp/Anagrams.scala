...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (ch, n) :: tail =>
      ...
      } yield (ch, i) :: occ).toList map noZeros
  }

  private def noZeros(list: Occurrences) = list filter (_._2 != 0)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnas(o: Occurrences): List[Sentence] = {
      if (o.isEmpty) {
        List(Nil)
      } else {
        ...
        for (i <- combs if dictionaryByOccurrences.keySet(i);
             ...
             s <- sentenceAnas(subtract(o, i))) yield {j :: s}
      }
    }
    sentenceAnas(sentenceOccurrences(sentence))
  }
}
